...
async function renderMovieInformation() {
    const selectedMovie = localStorage.getItem('selectedMovie'); //hämtar den valda filmens imdbID från localStorage som tidigare sparats vid klick
        if(!selectedMovie) { // om inget imdbID hittas så trycks ett felmeddelande ut och funktionen avslutas
        ...
        if (!movie || movie.length === 0) { //om filmen inte finns så trycks ett felmeddelande ut och funktionen avslutas
