...
export async function fetchTopMovies() {
  try {
    ...
    if (!response.ok) {
      //try/catch för felhantering, kastar ett fel om responsen inte är ok och skickar med .status
      throw new Error(`Something went wrong! Status: ${response.status}`);
    }
    let movies = await response.json();

    if (!Array.isArray(movies)) {
      //kollar att formatet är en array
...
async function fetchOmdbMovies(query) {
  try {
    let response = await fetch(
      `http://www.omdbapi.com/?apikey=a1dc276d&s=${query}`
    );
    if (!response.ok) {
      throw new Error(`Something went wrong! Status: ${response.status}`);
    }
    ...
    if (movies.Response === "False" || !movies.Search) {
      //vid sökning hämtas ett objekt som i sin tur innehåller en egenskap(Search) som innehåller en array.
...
async function fetchMovieInformation(query) {
  try {
    let response = await fetch(
      `http://www.omdbapi.com/?apikey=a1dc276d&plot=full&i=${query}`
    );
    if (!response.ok) {
      throw new Error(`Something went wrong! Status: ${response.status}`);
    }
