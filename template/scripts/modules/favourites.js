...
async function renderLikedMovies() {
  ...
  if (likedMovies.length === 0) {
    // kollar ifall listan är tom (ifall det finns några gillade filmer), om inte trycks ett felmeddelande ut och funktionen avslutas
    document.querySelector(
      "#favoritesContainer"
    ).innerHTML = `<p class="favorites__error">You dont have any favorites yet!</p>`;
    return;
  }

  for (let imdbID of likedMovies) {
    //loopar igenom alla imdbID i likedMovies
  ...
  setTimeout(() => {
    //sätter en kort timeout så att alla korten renderas innan lyssnaren för like-knappen sätts på
    likeButtonListener();
  }, 100);
}
